...
### 72.6 ✅⭐⭐ BOTH DECISIONS CLOSED — the plan is RATIFIED and the sides are left=G right=B; and the frames show the top camera filming a laptop lid

✅⭐ **His words, 2026-08-19: "I think so" to both.** Recorded as: **[docs/PLAN.md](PLAN.md) is ratified** (he had read it the same day: *"the plan in general looks good"*) — the draft is now the team's deliverable. And **the bench sides are left = G, right = B**, the mapping both his exports already used; every future episode inherits it.
...
✅⭐ **THE SWEEP THAT FOLLOWED FOUND FOUR MORE PLACES, and one of them would have broken the first health command on the new machine.** Grepping for platform assumptions rather than waiting to hit them:
1. ⛔ **`open_motor_interface` and `open_raw_can_interface` hardcoded `bustype="gs_usb"` with an integer adapter index.** They are the direct-bus openers that bypass `chain_channel` — and **`apps/ping_motors.py` goes through the first one, which is step 2 of the README's bring-up checklist.** Both now ask `bus_selector(arm)`, which returns SocketCAN kwargs on Linux and the gs_usb index on macOS. It also returns whether there is a serial to re-verify after opening: on macOS yes (the index could go stale between scan and open, [§0](FINDINGS.md) #5), on Linux no, because the interface name was itself derived from the serial and SocketCAN exposes no USB serial through the socket.
2. **`apps/camera_view.py --list`** would have printed *"macOS reported no cameras"* and then probed every index for nothing. On Linux it prints the by-id table instead, which is the same information without the theatre.
3. **`resolve_camera()`**, which `apps/capture_probe.py --cameras` also uses, now dispatches to the Linux resolver — so two tools were fixed by one change.
4. **`apps/probe_can.py` REFUSES on Linux** with the equivalent `ip link ... listen-only on` + `candump` recipe. It uses libusb to reach gs_usb's listen-only mode, and on Linux the kernel owns the adapter, so it would have failed as an unrelated-looking USB error. A tool that names the alternative beats a traceback.
...
✅⭐ **The batch pipeline came with it, because "everything converted, in a good order" was the actual ask:** `--all` exports every camera-carrying recording into `recordings/datasets/<split>/`, naming what it skips and why rather than passing over it (his shelf: slots 3-5 exported, slot 6 refused as one-arm, slots 1-2 refused because their role mapping names cameras they do not carry, slots 7-9 skipped as frameless) · `--split train|val` because the guide requires the validation set to be SEPARATE EPISODES from the start · and `apps/build_dataset_stats.py` writes the `norm_stats.json` the trainer looks for (C5), computed over the **train split only** so the held-out set cannot leak into every training batch. ⭐ **It verifies its own arithmetic**: it normalises the training rows with the statistics it just wrote and checks the result is zero-mean and unit-variance (worst |mean| 7.2e-12 on his three episodes). ⭐ **And it found something real about his data**: four columns never moved (the left arm's joint 2 and its jaw), because arm G held still in all three takes — floored to 1e-6 and NAMED, since a motionless column normalised by a floor becomes amplified noise. **51/51 contract checks across the three episodes.**
...
2. ⬜ **Whether to build a librealsense capture backend**, which is the only route to depth ([§76.10](FINDINGS.md)). No password needed, and nothing needs it yet.
3. ⭐ **Decide the frame-rate-against-brightness trade before collecting real data** ([§76.16](FINDINGS.md)). The station's C920 is currently allowed to halve its own rate in a dim room, so an evening demonstration carries half the images of a midday one. One command, no root, reversible: `v4l2-ctl -d /dev/video0 --set-ctrl=exposure_dynamic_framerate=0`. ⚠️ It costs image brightness, which is why the software reports it rather than deciding it. **Whatever he chooses, choose it once and keep it**, because the hazard is the inconsistency rather than either value.
