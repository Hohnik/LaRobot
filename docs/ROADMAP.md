...
2. ✅ **DONE AND CONFIRMED ON THE ARM, 2026-08-14.** `--arms` exists, `a` aims the mode keys B → G → BOTH, and the status is one row per arm — three commits (`9aae3a8`, `2a2dab8`, `8557e76`). Julien: *"Everything seemed to work as it should."* ([FINDINGS §53.0](FINDINGS.md)).
   - ✅ **The per-cycle loop is N-arm too**, in eight further commits (`b33ff82` … `fbb6945`): the per-arm helpers take an arm · per-arm control frame · per-arm axis map · per-arm base pose and slots · one puck per arm with `exclude` · per-arm CONTROLS and button state · liveness and the thermal guard per arm · the whole mode action per arm. **Every per-arm piece of state is on `ArmSession`, 33 fields, proven by `check_restructure.py`.** ⬜ **Not yet on the arm** — the owed test is one question and it is at the top of [HANDOFF](HANDOFF.md).
   - ⬜⭐ **THREE COMMITS REMAIN before `--arms B,G` can start, and the design for all three is settled in [FINDINGS §53.6](FINDINGS.md):** the key dispatch aimed at the selection (with the per-key classification table) · the shutdown and consent flow over N arms (⛔ `park_and_wait()` advances every arm per cycle rather than one after another, and it has no tests today while being the path Ctrl-C, `q q` and every unplanned stop run) · build N robots and drop the refusal, adding three refusals in the same breath (`--start-mode guide` at N>1, `w`/`l` at N>1 until the two-arm recorder exists, and deleting the test that pins the current refusal).
   - ⛔⭐ **The trap that decided the key design, found by reading `src/axis_map.py`:** `AxisMapStore.for_arm()` returns the **same** `AxisMap` object to both arms whenever the scope is SHARED, which is the default. So "apply this edit to every selected arm" would flip a motion **twice** — back to where it started — while printing two confirmations. **Map edits therefore apply to the FIRST selected arm only**, and the scope note already says whether that reaches the other arm. [FINDINGS §53.5](FINDINGS.md).
...
| # | what | state |
|---|---|---|
| 1 | ⏳⭐⭐ **Both arms from one script — STEPS 1 AND 2 DONE AND CONFIRMED, THREE COMMITS LEFT, 2026-08-14** | ✅ **Step 1**: `ArmSession` wired in, all **247** references, every mode driven by Julien ([FINDINGS §50](FINDINGS.md), [§51](FINDINGS.md)). ✅ **Step 2**: `--arms`, the `a` selector and one row per arm, confirmed on the arm — *"Everything seemed to work as it should."* ([§53.0](FINDINGS.md)). ✅ **The per-cycle loop is N-arm**, eight further commits, every per-arm field on the class (33 of them) — ⬜ **not yet on the arm**, and that test is owed BEFORE the next block ([§53.7](FINDINGS.md)). ⬜⭐ **Three commits left**, design settled in [§53.6](FINDINGS.md): the key dispatch aimed at the selection · the shutdown over N arms · build N robots and drop the refusal. ✅ **Arm G is free** as of 2026-08-14, so step 3 is unblocked once those land. Verify any step with `uv run scripts/check_restructure.py`. [ROADMAP §6.1](#61-audit-before-starting-2026-08-13--armsession-has-fallen-a-day-behind-and-the-plan-below-was-not-executable-as-written) |
...
| 21 | ⚠️ **The throttle message names a cause it never measured** | ⛔ New 2026-08-14. `⚠️ SLOWED to 19% (near the reach limit)` printed while the arm's manipulability was **0.1713**, which that throttle's own docstring calls *"middle of the workspace, comfortable"* ([FINDINGS §41.2](FINDINGS.md)). **The line shows the throttle percentage and not the speed setting it is throttling**, so the number cannot be read either way. ⭐ **Fix: show the linear speed and the effective speed, and say what was measured rather than what is assumed.** Small, and it belongs with any other status-line work |
