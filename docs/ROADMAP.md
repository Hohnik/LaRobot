...
| 47 | ⬜⭐⭐ **An arm WITHOUT a puck should still join a session — the next build item** | ⛔ **With the second SpaceMouse gone, `--arms B,G` refuses outright** (*"no unassigned SpaceMouse left for G"*), which kills MIRROR and two-arm playback — although a mirror follower and a replaying arm never need a puck ([FINDINGS §68.5](FINDINGS.md)). ⬜ Give the puckless arm a null reader (zero deflection): HOLD/GUIDE/playback/mirror-follower fully usable, only its own TELEOP dead, said plainly in the plan |
