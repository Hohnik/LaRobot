...
1. ⬜⬜ **`--arms B` runs the N-arm code with N=1, and its size is now measured.** `main()` is **1806 lines** (500-2305) with **338 references** to the 20 state names that become `arm.<field>`; `mode` alone is **93 sites** ([FINDINGS §36.3](FINDINGS.md)). ⛔ **Land it as one mechanical change with NO behaviour change**, so the hardware test asks exactly one question: were 338 substitutions made correctly? Julien then confirms it **feels identical**, which verifies the restructure against a feel he already knows, separately from any two-arm risk.
...
| # | what | state |
|---|---|---|
| 1 | ⏳⭐⭐ **Both arms from one script — STEP 1 OF 6 IS DONE AND VERIFIED, 2026-08-14** | ✅ **`ArmSession` is wired in**: all **247** references to one arm's state go through it, landed as five commits, and Julien has driven **every mode** on the result ([FINDINGS §50](FINDINGS.md), [§51](FINDINGS.md)). ⛔ **`--arms` still does not exist** — step 1 made the state *shape* right for N arms and nothing more. ⬜ **Step 2 is the next engineering work in the project**: add `--arms`, the `a` selector (B → G → BOTH) and per-arm status rows. ⚠️ **Step 3 is the first that needs arm G**, which a colleague borrows ([FINDINGS §35.6](FINDINGS.md)). Verify any step with `uv run scripts/check_restructure.py`. [ROADMAP §6.1](#61-audit-before-starting-2026-08-13--armsession-has-fallen-a-day-behind-and-the-plan-below-was-not-executable-as-written) |
