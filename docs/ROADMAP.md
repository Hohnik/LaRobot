...
| 39 | ⏳⭐⭐⭐ **The mirrored arm sits ~2 cm from where it should be — CAUSE FOUND AND A FIX BUILT, 2026-08-17, unrun** | ⭐⭐ **Julien's question, and the answer is both halves.** The follower is position-controlled, so it settles where its motor force balances gravity and friction, always SHORT of the command — the 0.04-0.10 rad constant in item 11's measured law. ⛔ **And nothing in software ever read that back**: `follower_target()` copies the leader's angles and the command converges to exactly them, so the follower sits at `leader − droop` forever. ⭐ Measured: 0.024 rad of joint error is **11.3 mm at the tip** in his reaching pose, so his 2 cm sphere is arithmetic rather than an impression. ✅ `--mirror-catchup` (setting 7 on the `n` screen) accumulates the error into a clamped bias and aims past the leader. ⛔ **DEFAULT OFF** — it changes what 4.3 kg does. ⬜ **Needs one hardware run: `--mirror-catchup 3` and see whether the sphere closes** ([FINDINGS §64.0](FINDINGS.md)) |
