...
4. `apps/` + `checks/` + `tests/` split, `check_flags` updated for new paths, docs updated the same commit. ⭐ **Two traps analysed in advance (2026-08-18), so the executing session starts from answers:** ① four test files `import teleop_session` as a SIBLING and two source-pin tests read it by path — when apps and tests separate, those tests need `sys.path.insert(REPO / "apps")` (contained to tests that test the app script; the library needs no hacks) and the pin paths updated; `check_restructure`'s TARGET moves too. ② `check_flags` validates `uv run` lines in ALL of `docs/*.md`, dated FINDINGS entries included — so moving scripts breaks validation of historical command lines. **Lean: re-path command LINES everywhere, including dated entries** — a documented command's job is to be runnable, unlike prose paths, which keep their day's truth ([FINDINGS §70.7](FINDINGS.md)'s convention governs prose only).
