...
| # | what | state |
|---|---|---|
| 1 | ✅✅⭐⭐⭐ **Both arms from one script — DONE AND RUNNING ON THE HARDWARE, 2026-08-14** | ✅ Steps 1-3 complete and confirmed: `ArmSession` (247 references), `--arms B,G`, and the first two-arm run — *"Wow. Everything seems to work."* ([FINDINGS §50](FINDINGS.md), [§51](FINDINGS.md), [§53](FINDINGS.md), [§54](FINDINGS.md), [§55](FINDINGS.md)). **502 headless tests, eight mechanical checks.** ⭐ 91 Hz with two arms, against 83-88 with one. ⏳ **Step 5, MIRROR, is built and never run** — ten-minute procedure in [§55.7](FINDINGS.md). ⏳ Step 4: GUIDE was entered on each arm separately; BOTH at once is untested. ⬜ **Step 6, the two-arm recorder, is the remaining piece** and it is item 7 of this table. ⚠️ Three things refuse with two arms on purpose: `--start-mode guide`, `w`/`l`, and `m` while BOTH is selected. [ROADMAP §6.1](#61-audit-before-starting-2026-08-13--armsession-has-fallen-a-day-behind-and-the-plan-below-was-not-executable-as-written) |
