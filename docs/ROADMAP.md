...
### 6.2 ⭐⭐ The park-collapse audit frame — measured 2026-08-18, the prep for item 23

> ⛔ **Read this before starting item 23, then audit each site fresh — this frame says what KIND of trap each site holds, not that the traps have been counted.** The jaw pause (items 3, 10) waits on this collapse, because its segments run through the park machinery and building on a duplicated park means building on the wrong copy.

**The measured facts (2026-08-18):**

- **36 call sites** of the four script-side functions (`enter_hold` · `enter_guide` · `enter_teleop` · `begin_path`) in `scripts/teleop_session.py`.
- ⛔ **The mode side-effect is the trap, confirmed at the definitions:** the class's `enter_hold()` ends with `self.mode = "hold"`; the script's closure does NOT touch mode, and callers set it themselves — sometimes to something else (`arm.mode = "park"` before calling `enter_hold`, [FINDINGS §52.1](FINDINGS.md)). **A naive substitution at such a site leaves a park running in HOLD.** Every site needs the question: *what does the caller set mode to, and when?*
- **`enter_teleop` differs by construction:** the script builds `CartesianTeleop(frame=one.frame)` inline; the class takes a `teleop_factory`. The migration passes `lambda frame: CartesianTeleop(frame=frame)` — behaviour-identical, and the factory is why the class is testable.
- **`enter_guide` differs in error style:** the class RETURNS a warning string when the zero-gravity API is missing; the script prints. The caller must print the return, or the "staying in HOLD (NOT weightless)" warning is silently dropped — on the mode where a silent failure means 4.3 kg held only by a message that lied once before.
- **The park machinery is the big half:** the class's `begin_path`/`step_path` (blended path, arc-length cursor, lag hold, stall guard, 45 tests) versus the script's `park_arms()` (line ~553, quit/Ctrl-C/auto-park) plus the in-loop park advance (`p` runs). These are two implementations of the same motion; the tested one never runs ([FINDINGS §52.1](FINDINGS.md)).

**Order of work, one group per commit, checkers green between:** ① `enter_hold` sites · ② `enter_guide` sites · ③ `enter_teleop` sites · ④ the path/park machinery. **Then one bench pass over all modes — Julien's, and nothing builds on the collapse until it has run.**
...
**✅ BUILT 2026-08-18, all four, on the merged park ([FINDINGS §70](FINDINGS.md)) — kept as the spec it was:**

1. ✅ `begin_path()` builds one `JointPath` **per segment** and keeps a queue (`ArmSession.park_queue`).
2. ✅ On reaching the end of a segment, the jaws are commanded and waited for. ⭐ **Waiting is measured, not timed**: the run proceeds when the jaws stop moving (still 0.35 s), which also covers a jaw that stalls on the object. The timeout exists (3 s), is reported, and continues rather than gating for ever.
3. ✅ **The gripper is commanded through `clamp_gripper`** (every waypoint passes `park_target_from` with the clamp) **and additionally through the `hold_jaw` block latch** — the old park bypassed the latch and would push into a held object at 90 Hz ([FINDINGS §70.1](FINDINGS.md)).
4. ✅ The plan line counts the stops before Enter (*"⏸ 1 jaw stop"*), and the MODE line says *"pausing 1× for the jaws"*.

⚠️ The "own session" this section asked for was the item-23 park merge, which had to land first — the pause was then built ON the merged `step_path`, one session later. ⬜ **Unrun on hardware; the 3-minute procedure is [FINDINGS §70.3](FINDINGS.md).**
...
| 3 | ✅✅⭐⭐ **A pause where the jaws move — CONFIRMED ON HARDWARE 2026-08-18** | ✅ **His pass ran the grab sequence twice**, the pause held, waited a measured 0.6 s, and resumed — *"pretty much worked… nearly no issues"* ([FINDINGS §70.5](FINDINGS.md)). The grab itself missed by millimetres: the friction floor ([§69.2](FINDINGS.md)) plus jaws that started while the arm still crept. ✅ **The settle-gate now holds the jaws until the arm settles and prints the settled offset** — the miss-diagnosis number. The dataset answer stays item 12 (grab legs taught by hand) | ✅ **`ArmSession.begin_path`/`step_path` run the segments now** ([FINDINGS §70](FINDINGS.md)): one blended path per segment, the arm holds at the split, the jaws are commanded through the block latch and **waited for by measurement** (still 0.35 s = done; a jaw stalled on an object counts as done; 3 s timeout says so and continues). The plan line counts the stops before Enter. ⭐ Two defects fixed in passing: the park had been pushing into a latched object at 90 Hz, and `err`/`lag` counted the jaw so a successful grab would have read as a BLOCKED park — both arm-only now ([FINDINGS §70.1](FINDINGS.md)). 15 tests (`test_jaw_pause.py`), sim drive 25/25. ⬜ **Owes one 3-minute bench pass — the procedure is [FINDINGS §70.3](FINDINGS.md)** | **half built 2026-08-13, and the design turned out better than a dwell time.** ✅ `src/motion.py::plan_gripper_stops` decides where a run must split, with 9 tests. ⬜ The park machinery has to run the segments and wait for the jaws. ⭐ **No configuration is needed**, which is why this is now smaller than it looked. [ROADMAP §6.6.2](#662-a-pause-where-the-jaws-move--built-as-a-decision-2026-08-13) |
...
| 5 | ✅✅⭐⭐ **Telling two identical D405s apart — CLOSED END TO END 2026-08-19** ([FINDINGS §70.15](FINDINGS.md) + [§71.5](FINDINGS.md)): the uniqueID IS locationID·VID·PID (verified on all three cameras; `yam/cameras/identity.py`, tests on a real-dump fixture), and the last link — uniqueID→OpenCV-index — is PINNED from his viewpoint confirmation (probe PNGs: index 1 = the G-view D405 `260323072846`, index 2 = the B-view `255323071773`), stored in `config/camera_index_hint.json`. ⛔ The pinning found the hint file already POISONED (both D405s → the C920's index) and the session now MODEL-CHECKS every hint-resolved index before trusting it. Serials accept unique prefixes (`d405:2553`). On Ubuntu librealsense reads serials directly and the question dissolves. ⭐ Bonus correction: camera ENUMERATION needs no macOS permission, only capture does | ⏸ **The problem cannot exist with one D405** (a D405 and a C920 are different models), so this moves into the consolidation plan as the rebuild's first camera task ([FINDINGS §67.12](FINDINGS.md)). What the plan carries: the two D405s DO have distinct USB serials readable with no root ([FINDINGS §34.5](FINDINGS.md)); the unsolved half is serial→OpenCV-index; the always-works fallback is capture-and-cover-one-lens | nothing exists. Use the wiggle approach, [FINDINGS §28.5](FINDINGS.md). ⛔ **NEEDS BOTH D405s ATTACHED — only one was on 2026-08-17.** The whole problem is distinguishing two identical devices. ⛔⭐ **And step zero for EVERY camera item: macOS camera access is NOT granted**, so OpenCV opens nothing at all ([FINDINGS §60.4](FINDINGS.md)) |
| 6 | ✅✅⭐⭐ **Capturing several cameras with timestamps — MEASURED ON THE RIG 2026-08-19** ([FINDINGS §71.4](FINDINGS.md)): his `capture_probe --indices 0 1 2 --seconds 10 --save` run answered the bandwidth question — **all three cameras hold 30.0 fps at 1280x720 simultaneously**, worst blind gap 66 ms (one skipped frame), fresh ~40% of 90 Hz samples (the expected ~1/3 for 30 fps sources). `yam.cameras` underneath: the LaRobot-aligned `Frame` (device timestamp honestly `None`, host stamp at frame-STORE time), the confirmed `FrameGrabber` (one copy, the viewer imports it back), `CaptureSet` with per-camera sequences so a repeat is visible, never silent padding. 8 headless tests + the hardware run | Nothing exists in code yet. Images must line up with joint data or the dataset is unusable. ⭐ **The design decisions, so the building session starts from answers:** ① the frame record aligns field-for-field with LaRobot's `Frame` ([§10.6](#106--what-the-colleagues-larobot-branches-show--explored-2026-08-18-his-instruction)): `camera_name` · `sequence` · `camera_timestamp_ns: None`-able · `host_timestamp_ns` · `rgb` · `depth: None` — depth is always None on this stack ([FINDINGS §63.0](FINDINGS.md)) and OpenCV's device timestamps on macOS are unreliable, so `camera_timestamp_ns=None` + a host stamp taken at grab-return is the HONEST record, never a fabricated one. ② **One reader thread per camera, latest-frame-wins**: `read()` blocks tens of ms and three cameras polled serially would eat the 90 Hz loop; the session samples each camera's latest frame per cycle. ③ **Config keys the C920 by MODEL and the D405s by serial** — the C920's USB serial is EMPTY ([FINDINGS §70.6](FINDINGS.md)), and serial→OpenCV-index for two identical D405s stays item 5 (operator-established mapping, verified per open by the resolution probe at model level). ④ The one command he runs measures achieved fps per camera and the host-timestamp spread — two-camera USB bandwidth failure shows as dropped frames, not as an error. New code lands as `src/yam/cameras/` (the folder is created BY this item, deliberately not before) |
| 7 | ✅⭐⭐ **The episode export — BUILT 2026-08-19 against the C3 contract** | ✅ `yam/episode.py` + `apps/export_episode.py` ([FINDINGS §70.13](FINDINGS.md)): exact topics/dims/tick, next-tick action policy named in the episode's own metadata, labels + provenance + the REQUIRED left/right mapping in `/episode-meta`, single-arm and wrong-mapping refusals, 8 read-back tests. Ran end to end on his real slot-8 recording (190 ticks, 6.3 s). ⬜ **Two open halves, both named in the export's own warnings:** camera frames are not wired into the recorder yet (the capture layer exists, the session does not sample it while recording — needs the camera flags + one bench session), and the Anleitung's **C4 gate** (verify a mini-sample against ABC's loader) is the team's step, not this repo's | nothing exists. Metadata list in [ROADMAP §6.6](#66-where-the-training-data-comes-from) |
| 8 | ✅⭐ **Good/bad labels while driving — the KEYPRESS half is BUILT, 2026-08-19** | ✅ `k` while recording toggles bad↔good from that moment; a recording starts implicitly good, marks live in the file's `meta` (old files read as all-good), `Trajectory.label_spans()`/`bad_seconds()` are the export-facing API, and `check_recordings.py` shows labelled slots. 4 tests ([FINDINGS §70.10](FINDINGS.md)). ⬜ Owes one feel-run (record, press `k` twice, check the stop summary and the listing). ⏸ The microphone half stays future — it needs an audio capture thread and voice-to-label mapping nothing else needs yet | nothing exists. Keypress first, microphone second |
| 9 | ⬜⭐ **Noise per waypoint — mechanically READY, gated on ONE decision that is HIS** | ⭐ With composite runs built ([FINDINGS §70.12](FINDINGS.md)), his §6.6 rule is implementable as a filter: noise applies to pose-leg waypoints EXCEPT any waypoint that borders a jaw pause — `plan_gripper_stops` already computes exactly which those are, so "the grab waypoint stays exact" needs no new bookkeeping. ⛔ **The open decision is the SAFETY one, and it is his:** a noised waypoint is a pose the arm has never physically held, which deliberately breaks the measured-pose property parks rely on ([FINDINGS §37.3](FINDINGS.md)). He must set the bound (agent lean: ±0.03 rad per arm joint, never the jaw, clamped inside joint-limit margins, OFF by default) before any noised target reaches an arm. ~2 minutes of his time, then one hardware-free build session | nothing exists |
| 10 | ✅✅⭐⭐ **Detecting whether a grab worked — CONFIRMED ON HARDWARE 2026-08-18, with a TRUE NEGATIVE** | ✅ Its first real answer: *"∅ the jaws closed onto themselves — nothing gripped"*, and the piece HAD been missed ([FINDINGS §70.5](FINDINGS.md)). A failed grab that announces itself is the dataset filter working | ✅ **The jaw pause calls it on every closing leg's resume cycle** and the session narrates the verdict: *"✋ holding something — the jaws stopped 0.200 of the stroke short of closed"* or *"∅ closed onto themselves"* ([FINDINGS §70.0](FINDINGS.md)). A timed-out pause passes `settled=False`, so the check refuses to answer rather than guessing — its own rule, kept. ⬜ Confirmed by the same owed bench pass as item 3 | **built 2026-08-13.** `src/yam_robot.py::check_grasp`, 7 tests. ⭐ It needs no new hardware and no per-object calibration: calibration closes the jaws onto themselves, so a normalised 0 means empty and anything above it is an object's width. ⛔ It refuses to answer unless the jaws were told to close AND have stopped moving, because mid-close looks identical to holding a wide object. ⬜ Nothing calls it yet; it needs the pause in item 3 first, since that is where a run waits for the jaws |
...
| 12 | ✅⭐⭐ **Mixed runs — PHASE 2 BUILT 2026-08-19, sim-confirmed 29/29 first try** | ✅ `p 1 w8 1 Enter Enter` parks, plays take 8, parks again ([FINDINGS §70.12](FINDINGS.md)): takes validated at Enter before any motion, the queue advances only in arrival/completion branches, one `abandon_composite()` guards every exit, and the take leg IS the confirmed `l` flow via shared closures. ⬜ Owes one composite grab on the bench. ⏸ Phase 3 (teaching a leg MID-run) waits for that | nothing exists. His idea, and it turns the noise rule into a data structure. Needs 3 first. [ROADMAP §6.6.1](#661-mixing-waypoints-and-recordings-in-one-run--his-idea-2026-08-13) |
| 13 | ✅⭐⭐ **The puck scrub — CONFIRMED ON HARDWARE 2026-08-18, and the time-lapse dial is in** | ✅ **His verdict the same evening: "the scrub works", forward and backwards, two-arm and one-arm** ([FINDINGS §68.0](FINDINGS.md)). ✅ His follow-up ask built: `--scrub-max` + `-`/`+` during the scrub set the full-push pace, up to 8× time lapse — safe by construction, the lag hold means only the clock is fast ([§68.4](FINDINGS.md)). ⛔ `l` refuses during a running playback now, and the held line says "scrubbing… at Xs of Ys" ([§68.3](FINDINGS.md)) | ✅ **`j` at the play prompt scrubs the recording with the puck**: forward plays, back rewinds, release freezes (the spring centre is a deadman). Pace capped at the same 1.5× a plain playback may reach, lag hold in both directions, either puck works, park-to-start unchanged. 6 tests; the "third puck role" worry dissolved — the dial is a playback behaviour, not a device role ([FINDINGS §67.13](FINDINGS.md)). ⬜ Owes one feel-run: `l`, `j`, scrub both ways | ⬜ **Build it in the prototype.** His ruling, and it carries the general principle: *"I would build everything in this prototype so that we know how to do it, and then we can explain how to do it in the rebuild plan."* His note: *"should be quite easy… should be made intelligently in some way"* ([FINDINGS §67.7](FINDINGS.md)). His own ranking of the roles still stands: scrubbing a recorded movement first | nothing exists. **His idea, first raised 2026-08-13 and raised again 2026-08-14** (*"didn't I already tell you about this?"* — yes, and it was written up the same day). ⛔ **The old note said this needs the two-arm work first, and that overstated it**: only one arm is driven today, so the second puck is free, and the real cost is designing a third puck role twice. ⭐ **The most useful version by his own ranking, scrubbing a recorded movement, is the least entangled and could be done before step 2.** His call. [ROADMAP §7.6](#76-the-second-spacemouse-as-a-continuous-speed-dial--his-idea-2026-08-13) |
| 14 | ✅⭐⭐⭐ **ANSWERED 2026-08-20: the loop runs below 100 Hz because `time.sleep` returns LATE, and on macOS it returns ~1.9 ms late every pass.** The measurement that settles it takes no hardware: an **empty** loop with no arms, no cameras and no work at all, using this loop's own `time.sleep(max(0, dt - elapsed))` line, reaches **84.3 Hz on the Mac** and **97.3 Hz on the Linux station** — and a full simulated session with two arms, a recording and a playback also reaches 84 to 85 Hz on the Mac. **So the Mac's entire shortfall is the wait and none of it is the work**, moving anything off the loop cannot recover it, and the 87 Hz written all over this repo was a macOS number rather than a property of the system ([PERFORMANCE.md](PERFORMANCE.md) §2, [FINDINGS §78.5](FINDINGS.md)). ⬜ **What remains is a real measurement on the station with the arms attached**, which now happens by itself: every session prints its own pass numbers when it stops. ⚠️ Unexplained residue, and it is small: the 87 → 83 drift inside one afternoon on 2026-08-13. *The original entry, kept because its candidate list is what the answer had to beat:* ⛔ **83 and 84 Hz in the two runs of 2026-08-13 16:34, against ~87 Hz measured earlier the same day.** The warning fires correctly (its threshold is 92 Hz). Unexplained candidates: the **second D405 that appeared on the bus** ([FINDINGS §34.5](FINDINGS.md)), general machine load, the per-cycle tracking log. ⭐ **Every playback now writes `loop_hz` into its saved tracking file**, so this becomes a trend rather than an anecdote. ⚠️ **It also makes a constant wrong:** `MAX_PLANNED_JOINT_SPEED` is `0.015 × 100`, so the real teleop ceiling is ~1.25 rad/s not 1.5 ([FINDINGS §34.2](FINDINGS.md)). [FINDINGS §31.1](FINDINGS.md) |
...
| 21 | ✅⭐ **The throttle message named a cause it never measured — FIXED 2026-08-18** | ✅ The SLOWED line now prints the measured pair (the joint rate the IK asked for, against the cap): `SLOWED to 19% (joints asked for 7.9 rad/s, cap 1.5)`. Reading it: spikes only when extended = a singular pose; high everywhere = the linear speed outruns the joints. A test forbids a guessed cause from returning ([FINDINGS §67.11](FINDINGS.md)) | ⛔ New 2026-08-14. `⚠️ SLOWED to 19% (near the reach limit)` printed while the arm's manipulability was **0.1713**, which that throttle's own docstring calls *"middle of the workspace, comfortable"* ([FINDINGS §41.2](FINDINGS.md)). **The line shows the throttle percentage and not the speed setting it is throttling**, so the number cannot be read either way. ⭐ **Fix: show the linear speed and the effective speed, and say what was measured rather than what is assumed.** Small, and it belongs with any other status-line work |
...
| 38 | ✅⭐ **Two pending prompts could be open at once — FIXED 2026-08-18, newest wins** | ✅ **His ruling was "do that sensibly": the newest prompt cancels the older one, with a printed line.** Button learning can only be armed while no keyboard prompt is open (prompt handlers consume every key), so one per-cycle check suffices: a prompt opening while learning is armed cancels the learning ([FINDINGS §67.11](FINDINGS.md)) | ⚠️ **New 2026-08-17.** His log has the gripper-button learning (`b`) and the SETTINGS screen (`n`) both armed together. They coexist because `b` waits on a PUCK button while SETTINGS reads the KEYBOARD, so neither blocks the other, and **a puck press he made for one was consumed by the other**. ⛔ Harmless today: nothing moves and no setting is corrupted. ⚠️ But two invisible modal states is the shape that produces a *"why did that do nothing?"* session later. ⭐ **Needs Julien's decision on which should win** before it is built ([FINDINGS §63.3](FINDINGS.md)) |
| 39 | ✅⭐⭐⭐ **The mirrored arm sits ~2 cm short — UNDERSTOOD 2026-08-18 late: it is the STATIC-FRICTION floor** ([FINDINGS §69.2](FINDINGS.md): gravity is already compensated by the vendor, the measured droop × measured m/rad = his 2 cm, teleop only feels exact because his eyes close the loop; the one real lever is kp = item 17, his call; recommendation: accept as the repeatability floor, state it in the plan) | ✅⏸ **Ran on hardware, reported 2026-08-18, and the feature is weak by his own verdict**: jittery and shaking at high values (20), slow aimless drift even at low ones (*"it doesn't really work that well"*). ⭐ Both behaviours are what an integrator aimed at a moving noisy target must do, so the design is the limit rather than a bug ([FINDINGS §67.1](FINDINGS.md)). ⛔ Default stays OFF, the 20 ceiling stays (his ruling, same report). **The real fix for the droop is velocity feedforward, item 44** | ⭐⭐ **Julien's question, and the answer is both halves.** The follower is position-controlled, so it settles where its motor force balances gravity and friction, always SHORT of the command — the 0.04-0.10 rad constant in item 11's measured law. ⛔ **And nothing in software ever read that back**: `follower_target()` copies the leader's angles and the command converges to exactly them, so the follower sits at `leader − droop` forever. ⭐ Measured: 0.024 rad of joint error is **11.3 mm at the tip** in his reaching pose, so his 2 cm sphere is arithmetic rather than an impression. ✅ `--mirror-catchup` (setting 7 on the `n` screen) accumulates the error into a clamped bias and aims past the leader. ⛔ **DEFAULT OFF** — it changes what 4.3 kg does. ⬜ **Needs one hardware run: `--mirror-catchup 3` and see whether the sphere closes** ([FINDINGS §64.0](FINDINGS.md)) |
...
| 43 | ✅⭐ **The `n` screen hid the status row — FIXED 2026-08-18** | ✅ **Every `-`/`+` press now prints each arm's live status row under the change** (mirror note included), so the effect of a setting is visible per press. It bit twice before the fix: 33 presses tuning `mirror_catchup`, then `vel_ff` tuned blind the same evening the row landed ([FINDINGS §67.10](FINDINGS.md)). ⭐ Bonus from the same paste: `q` inside the screen now closes it and hands over to the quit flow — he pressed it twice wanting exactly that | ⚠️ **New 2026-08-17.** He pressed `-`/`+` on `mirror_catchup` **33 times** — 3, 5, 8, 12, 8, 5, 3, 2, 1, 0, 1, up to the ceiling and back down twice — because the row showing the correction was covered by the screen he was tuning it on. ⭐ **A live editor whose effect is invisible while editing is half a feature.** ⬜ Options: keep one status line visible under the screen, or make the settings block shorter than the terminal so both fit. ⚠️ Also reconsider `i` inside the screen, which he pressed wanting to re-engage mirror ([FINDINGS §65.4](FINDINGS.md)) |
| 44 | ✅⛔⭐⭐⭐ **Velocity feedforward — CLOSED 2026-08-18 night: capped at 1, and above 1 is a measured dead end** | ✅ **His second verdict killed the exaggeration range**: mitigated gain 3 *"basically moving in steps… five and ten steps per second… like the motor's vibrating"*, even during parks. ⛔ **Above 1 the setpoint contradicts the position command by construction — no gate or filter can smooth it without becoming gain 1** ([FINDINGS §68.6](FINDINGS.md)). ✅ `VEL_FF_CEILING` is 1.0; the gate and smoothing stay (they help at ≤ 1); the setting he confirmed as precise (*"one basically doesn't change the direction at all"*) is the ceiling. ⭐ More responsiveness, if ever wanted, is a kp/kd question (item 17's caveats), carried into the plan | ✅ **He felt every level and asked what gain 3's pull-back was: it was the arm returning from OVERSHOOT, never a backwards control** — full mechanism per level in [FINDINGS §68.1](FINDINGS.md), and his "1 basically doesn't change direction at all" is the physically-correct value confirming itself. ✅ **Mitigation built: a joint past its command gets zero push immediately, and the setpoint is smoothed over ~2 cycles** (release decays instead of cutting; `resync()` clears it). ⬜ **Owes a re-run at gain 2-3: jitter and pull-back should both be visibly smaller** | ✅ **Ran at 0.25 on both arms, TELEOP, 161 s**: *"feels different… maybe that's just because now they're actually more direct."* ✅ His ask implemented: ceiling 1 → **3** so the effect stops being subtle — 1 stays the physically-motivated rung, above it is labelled exaggeration with overshoot expected ([FINDINGS §67.10](FINDINGS.md)). ⬜ **Owes a visible verdict**: the same movement at 0 and at 2-3, watching FOLLOWING | ✅ **Built the same day as his go** ([FINDINGS §67.9](FINDINGS.md)): `--vel-ff` / setting 9, OFF by default, the setpoint is the rate-limited command's own derivative (bounded by construction), the jaw excluded, 8 new tests, the sim loop 25/25 with it on. ⬜ **Owes one gentle hardware run at `--vel-ff 0.25`** — watch for buzz/overshoot, back off live with `n` `9` `-` | ✅ **His go: *"try to build everything else relevant, especially regarding the motor speed."*** Build off by default; he tests slowly ([FINDINGS §67.7](FINDINGS.md)). It is also the real answer to the weak catchup (item 39) | ⭐ **New 2026-08-17, from his latency question.** The DM motors' MIT-mode command carries `(kp, kd, position, velocity, torque)` and we send **velocity = 0**, so all torque must come from position error — which is exactly the measured `0.033 s × speed` lag ([FINDINGS §66.1](FINDINGS.md)). Sending the target's velocity lets torque flow before error builds. ⛔ Needs work in the I2RT interface layer and a careful bring-up (a wrong feedforward oscillates); **his ratification before it touches the arm** |
...
| 28 | ✅⭐ **Each arm's control frame shows on its status row — DONE 2026-08-18** | ✅ TELEOP's bracket carries the frame: `[B TELEOP/w]` · `/t` · `/c` for world · tool · camera, padded to the same 8 columns as CONTROLS so the rows stay aligned. Two arms driven in different frames are now distinguishable at a glance ([FINDINGS §67.11](FINDINGS.md)) | ⚠️ New 2026-08-15, small. `v` aims at ONE arm, so two arms can be driven in different frames — one in `world` while the other follows its own wrist in `tool`. **Nothing on screen says which is which.** The recording metadata records both since tonight; the live row does not. Cheap, and it only matters once he actually uses two frames at once |
...
| 23 | ✅✅⭐⭐⭐ **The park merge is DONE — bench pass CONFIRMED 2026-08-18 late (\"everything really feels exactly as before\"), [FINDINGS §69.0](FINDINGS.md)** | ✅ **All four groups of the [ROADMAP §6.2](ROADMAP.md) audit are in** ([FINDINGS §68.9](FINDINGS.md)): enter_hold (16 sites, two §52.1-trap reorders) · enter_guide (3 sites, the warning prints from the caller) · enter_teleop (4 sites, injected factory, one reorder) · **the park itself — `ArmSession.step_path` with its 48 tests finally moves the arm, and the script only narrates the ParkStep**. ⭐ The audit paid twice: the class's settle phase gained the script's newer wisdom (keep commanding + credit err progress, else feedforward freezes a stale setpoint and a slow settle reads as blocked), and `check_restructure` gained the tighter CLASS-INTERNAL rule, falsified before trusting. ⭐ `park_arms` (quit/Ctrl-C) deliberately stays: tested AND running, both halves of §52.1 false for it. ⬜ **His bench pass: every mode once** | ⛔ **New 2026-08-14, and it is the finding of step 2** ([FINDINGS §52.1](FINDINGS.md)). Checked rather than assumed: `teleop_session.py` calls exactly ONE method on `ArmSession` (`arm.alive()`). Its own closures still do the entering, the clamping and the whole park, so **`ArmSession.step_path()` and the 45 tests around it describe a park that never executes.** ⚠️ A reader who sees those tests pass would be wrong about the code that moves 4.3 kg, and nothing said so. ⭐ **Fix: delete the script's closures and call the class's methods**, one group per commit. ⛔ **NOT mechanical** — the script does `arm.mode = "park"` and then calls its own `enter_hold()`, which leaves the mode alone, while the class's `enter_hold()` sets `mode = "hold"`. A naive substitution at that one site leaves a park running in HOLD. Every site needs the same check, and the result needs one bench pass over all modes |
...
| 46 | ✅⛔⭐⭐ **An unplugged SpaceMouse DROPPED THE ARMS — FIXED 2026-08-18** | ✅ **His test: pulling a puck mid-session raised `OSError: read error`, the exception skipped the auto-park, and the finally disabled every motor where the arms stood** ([FINDINGS §68.2](FINDINGS.md)). The per-cycle read is guarded now: a dead puck reads as CENTRED and the stop routes through the SAFE STOP — park, then disable. His ruling: *"a graceful quit is probably good when something gets unplugged."* Source-pin test in `test_incident.py` |
| 47 | ⏳⭐⭐⭐ **One puck, N arms — CONFIRMED, and the puck FOLLOWS THE SELECTION now (his design, built 2026-08-18 night)** | ✅ **He confirmed the puckless arm joins** (*"the two arms with one puck works"*) and designed the next step in the same breath: with exactly one real puck, **`a` aims the puck as well as the mode keys** — B, G, or BOTH, where BOTH drives both arms at once, each from its own pose, no mirror needed (his call). Unaimed arms read a centred puck; the selection message says the puck moved; the shared reader is read once per cycle (one HID queue must not be drained by two arms); the unplug guard covers it ([FINDINGS §68.8](FINDINGS.md)). ⬜ Owes a feel-run: `a` through B → G → BOTH in TELEOP | ✅ When every attached puck is already assigned, the remaining arm joins with the zero-deflection `StillPuck` and a plain note: HOLD/GUIDE/playback/scrub/MIRROR-follower all work, only its own TELEOP is inert. ⚠️ **Gated deliberately**: an attached-but-unmoved puck still aborts, so an assignment the operator wanted never silently becomes a dead TELEOP. Source-pinned test | ⛔ **With the second SpaceMouse gone, `--arms B,G` refuses outright** (*"no unassigned SpaceMouse left for G"*), which kills MIRROR and two-arm playback — although a mirror follower and a replaying arm never need a puck ([FINDINGS §68.5](FINDINGS.md)) |
| 48 | ✅✅⭐⭐ **Camera frames INTO recordings — BUILT AND CONFIRMED ON HARDWARE 2026-08-19, same day** ([FINDINGS §71.2](FINDINGS.md) the build, [§71.4](FINDINGS.md) his runs): two camera-carrying takes recorded (153 and 262 frames at ~30 fps, ZERO drops), `check_recordings` counted them from disk, and slot 1 exported as the first episode WITH images (`/top-camera`, 154 ticks). The loop held 89-90 Hz with a camera; a no-camera session the same morning ran 83 Hz, so one camera's cost is invisible. ⬜ Remaining: the THREE-camera take (wrist hints pinned, [FINDINGS §71.5](FINDINGS.md)) and the C4 gate** | ⭐ **The design, so the building session starts from answers:** ① a `--cameras` session flag naming captures by the [FINDINGS §70.15](FINDINGS.md) identity chain (D405s by serial, C920 by model); opening happens in HIS terminal, which holds the permission. ② While RECORDING only, the loop samples `CaptureSet` and hands fresh frames to one writer thread per camera, which JPEG-encodes and writes `recordings/frames/<slot>/<camera>/<seq>.jpg` plus an index json of `(seq, host_stamp_ns)` — frames on disk, never in the recording json. ③ The recorder stamps `frames_dir` + per-camera counts into `meta`, and `check_recordings` shows them. ④ `yam/episode.py` merges them as the C3 camera topics by nearest-stamp join to each tick. ⑤ **What his first run must MEASURE, because no agent can:** the encode+write cost at 30 Hz × 3 cameras (the writer thread reports drops per camera, honestly — a silent drop is the [§0](FINDINGS.md) pattern), and whether the loop rate holds. ⛔ **The traps, named:** the writer must never block the 90 Hz loop (hand-off by reference, drop-oldest on backpressure WITH a count); a `--sim` session must refuse `--cameras` (a sim take with real frames is a mislabelling engine); teardown flushes indexes before the summary prints | ✅ built as designed: `yam/cameras/specs.py` + `writer.py`, the `--cameras` flag, the meta stamp, the `check_recordings` 📷 line + orphan scan, `nearest_frame_per_tick` + `--top`/`--left-wrist`/`--right-wrist` ([FINDINGS §71.2](FINDINGS.md)) |
| 49 | ⏳⭐⭐ **Run the whole station on the team's LINUX PC — his ruling 2026-08-19. Port WRITTEN, unverified** | ✅ `src/yam/platform.py` is the only OS branch in the repo: SocketCAN interfaces resolved from the adapter's USB serial in sysfs, cameras from `/dev/v4l/by-id` (model + serial in the name, and the symlink target IS OpenCV's index — so the [FINDINGS §70.15](FINDINGS.md) uniqueID chain, the hint file and the lens-covering are all macOS-only workarounds for a problem Linux does not have). `chain_channel` ported every whole-arm caller at ONE seam, and the vendor `dm_driver` needs no patch on Linux because its own `if "can" in channel` branch is finally correct. `linux_camera_for_spec` takes the Mac's three spellings and refuses ambiguity; recorded camera NAMES are identical across platforms. `checks/check_platform.py` reports tools, groups, per-interface CAN state with its sudo fix, cameras, the puck, pyrealsense2 — and MEASURES the perf_counter/monotonic epoch the frame join depends on. 10 tests + 2. ⛔ **Every Linux device path is designed-and-unverified: fixtures are hand-written from documented formats and labelled as such** ([FINDINGS §74.0](FINDINGS.md)). ⬜ Next: his ZeroTier/SSH steps ([docs/LINUX.md](LINUX.md) §1), then `check_platform.py --raw` on the real machine | ⛔ **New 2026-08-19.** The SDK supports Linux and not macOS; the whole walkthrough ran on macOS anyway with one argument-deep workaround; the real station is the Linux PC. So the port is a handful of device-naming differences, not a rewrite — and Linux answers most of them more simply |
| 50 | ✅⭐⭐ **The paper-standard training format (guide C4) — BUILT AND VERIFIED ON REAL DATA 2026-08-19** | ✅ `yam/dataset.py` + `apps/export_dataset.py` write `episode_<id>/` with `states_actions.bin` ((n,28) float64, columns named in the metadata), the camera views stacked vertically into one video encoded to the contract's exact properties, and `episode_metadata.json`. `checks/check_dataset.py` re-reads the result with ffprobe and asserts the CONSEQUENCE of every flag — **17/17 on his slot 5 recording** — and `checks/falsify_check_dataset.py` breaks it five plausible ways and catches **5/5**. One shared `state_action_rows` feeds the C3 log and the C4 set, so they cannot describe a demo differently ([FINDINGS §74.1](FINDINGS.md)). ✅ Plus the batch half of his ask: `--all` converts every camera-carrying recording, `--split train|val` keeps the validation episodes separate as C5 requires, and `apps/build_dataset_stats.py` writes C5's `norm_stats.json` from the train split only, verifying its own arithmetic and naming every motionless column. 51/51 across his three episodes. ⬜ Open, both the team's and both named in the output: the per-view size (guide says 224 square, their sim renders 224×168) and the gripper unit (normalised here, metres in their `Observation`) | ⛔ **New 2026-08-19, his ask**: recordings converted into "the correct file format that is used for all of the robotics papers", with the timecodes aligned. That format is the guide's own C4, and its encoding is strict BECAUSE the trainer computes where frame k is rather than decoding to find it — the guide measured a ~70× slower loader from getting it wrong |
...
✅⭐⭐ **DRAFTED 2026-08-19: [docs/PLAN.md](PLAN.md)** — phase map against the Anleitung, nine work packages, decisions, the method chapter, the dead ends; its three open inputs (the task, the model, one camera-integrated run) are open-decision sections inside it, exactly as this section prescribed. ⬜ His read + ratification makes it the deliverable. **PROMOTED 2026-08-18: this consolidation plan is THE endgame deliverable** ([FINDINGS §67.0](FINDINGS.md)). His ruling makes the sequence explicit: finish the features and the setup, then consolidate everything into a full plan — what is necessary, all the problems a rebuild will run into — and hand it to his team, who rebuild from scratch. The still-open decisions below (the task, the model, the papers) stop being *blockers* and become **open-decision sections inside the plan**, each with the evidence this repo already holds.
...
1. ✅ Ratified 2026-08-18 ([FINDINGS §67.7](FINDINGS.md)), and every open question is now answered: package `yam` (§10.6), and his 2026-08-19 ruling on language — **everything new in English, the German documents stay** (*"we are all fluent in both"*, [FINDINGS §70.8](FINDINGS.md)).
...
**Re-explored 2026-08-19 (fetched; PRs #13 and #14 landed on their main, the second that same morning — the team is building NOW):**

1. ✅ **Their `main` has a working MuJoCo simulation of the ABC `put_bottles` scene**: `src/robot/environment/simulation.py` — 30 Hz control ticks (17 physics substeps each), `reset()`/`step()`, cameras rendered at 224 wide, `realtime` wall-clock hold for a viewer, pytest coverage. `docs/ARCHITECTURE.mmd` states their plan as a class diagram: an `Input` interface (Keyboard · Spacemouse · **Policy**) and a `Robot` interface (RealRobot · SimRobot) — the policy-as-an-input idea is now their DECLARED architecture, not just a stub, and it is the same seam [PLAN.md](PLAN.md) WP3/WP9 names.
2. ⭐ **Their `Observation` dataclass fixes their data shape**: `joints (14,)` left arm then right arm, 6 joint angles in radians **then 1 gripper opening in METRES**; `images` keyed `top`/`left`/`right` (ABC's own `camera_keys`); `time` from reset. ⛔ **The gripper UNIT is a live contract risk**: theirs is metres, this walkthrough's episodes carry the jaw NORMALISED 0..1, and Setup-Anleitung C3 gives the ee dim without a unit. Flagged in [PLAN.md](PLAN.md) §4 — ABC's own `export_mcap.py` adjudicates at C4, and whoever loses converts.
3. ⭐ **Their `inputs/mcap_recording.py` is an EMPTY file** — the C3 recorder does not exist on their side yet. This walkthrough's episode exporter ([FINDINGS §70.13](FINDINGS.md), camera topics [§71.2](FINDINGS.md)) is the only running C3 implementation anywhere, which makes it the natural thing to hand over first.
4. ⚠️ `feature/camera-framework` now carries a real `Frame` dataclass and it matches ours field-for-field — with one field-name typo on their side (`camera_timestemp_ns`). Worth one friendly line to them: the lift from our `yam/cameras/` breaks on that name.
