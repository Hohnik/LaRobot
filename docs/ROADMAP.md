...
| 47 | ⏳⭐⭐ **An arm WITHOUT a puck still joins the session — BUILT 2026-08-18 evening, unrun on hardware** | ✅ When every attached puck is already assigned, the remaining arm joins with the zero-deflection `StillPuck` and a plain note: HOLD/GUIDE/playback/scrub/MIRROR-follower all work, only its own TELEOP is inert. ⚠️ **Gated deliberately**: an attached-but-unmoved puck still aborts, so an assignment the operator wanted never silently becomes a dead TELEOP. Source-pinned test. ⬜ **Restores MIRROR with one puck — one bench run confirms it** | ⛔ **With the second SpaceMouse gone, `--arms B,G` refuses outright** (*"no unassigned SpaceMouse left for G"*), which kills MIRROR and two-arm playback — although a mirror follower and a replaying arm never need a puck ([FINDINGS §68.5](FINDINGS.md)) |
