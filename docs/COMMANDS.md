...
```bash
uv run apps/teleop_session.py --yes --arms B,G --cameras c920 --start-mode hold   # frames ride every take
uv run apps/export_episode.py --slot 7 --left B --right G   # one MCAP episode, the team's C3 contract
uv run apps/export_episode.py --slot 7 --left B --right G --top c920   # ...with its camera topic(s)
```

⭐⭐ **`--cameras` records camera frames while a take records** ([ROADMAP §8.2](ROADMAP.md) item 48, [FINDINGS §71.2](FINDINGS.md)): comma- or space-separated specs — `c920` (resolved by measurement) · a raw index like `2` · `d405:<USB serial>` (the [FINDINGS §70.15](FINDINGS.md) identity chain; needs the one-time index confirmation in `config/camera_index_hint.json`). Cameras open at session start in YOUR terminal (macOS grants capture per app — [FINDINGS §61.3](FINDINGS.md)); while `w` records, each camera's fresh frames are JPEG-written to `recordings/frames/<slot>/<camera>/` by its own writer thread, with drops COUNTED, never silent. The recording's meta names the directory and the counts; `check_recordings.py` re-counts them from disk. ⛔ Refused with `--sim` (a sim take with real photographs is a mislabelling engine), and never saved by `--save-defaults`.
