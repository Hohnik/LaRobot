...
The walkthrough proved the whole loop on macOS with two YAM arms. That covers: SpaceMouse teleop at 90 Hz (the puck steers the gripper in cartesian space through inverse kinematics), hand-guiding (the arm goes weightless under gravity compensation and you move it by hand), waypoint runs that can grab (the run pauses where only the jaws move and reports whether something was gripped), recording and replaying hand-taught movements, composite runs (waypoints and recorded movements in one sequence), mirror mode (one arm follows the other joint for joint), marking good and bad stretches while driving, a simulator that lags like the real hardware and runs the whole session with nothing attached, and an exporter that turns a recording into an MCAP episode (MCAP is the log-file format your guide specifies) in exactly the topic shape of [Setup-Anleitung.md](Setup-Anleitung.md) C3. Two things do not exist anywhere yet: a collection run with all three cameras in the session, and an episode verified against ABC's loader (ABC is the training stack the episodes feed). The pipeline itself has run end to end on the real arms: record, label, export produced an episode with images on 2026-08-19, with zero dropped frames, and the USB tree carries all three cameras at 30 fps together ([FINDINGS §71.4](FINDINGS.md)). The most important thing we learned is not a feature. This stack fails by lying, never by crashing ([FINDINGS §0](FINDINGS.md)). §5 explains the defences that worked, because you will need the same ones.
...
The walkthrough deviated here completely and on purpose: everything ran on macOS, which the vendor SDK does not support. The workaround turned out to be one argument deep, not architectural (`bustype="gs_usb"` instead of libusb — [README §2.1](../README.md)). For your rebuild on Ubuntu, Phase A applies as written, and none of the macOS workarounds travel. One thing does travel: the [README](../README.md) bring-up checklist. It holds the motor LED table, the order for reading fault codes, which recovery needs a USB replug and which needs a mains cycle, and the gripper's per-session encoder shift. All of that is hardware truth, independent of the OS.
...
- The D405 over plain UVC (the ordinary webcam protocol) is a colour camera, full stop. Every mode was measured from the pixels ([FINDINGS §63.0](FINDINGS.md)). Depth needs the RealSense SDK, so on Ubuntu use librealsense from the start.
...
|---|---|---|---|
| 1 | CAN/robot layer: `build_robot`, `SafeRobot` (rate and lag clamps below all logic), teardown order | `yam/robot.py`, `yam/can.py` concepts; on Ubuntu the SDK's SocketCAN path replaces the gs_usb workaround | the vendor's own `close()` gets the teardown order wrong, and a raised arm sags if you trust it ([README §5](../README.md)) |
...
| 7 | Episode export and the C4 gate: recordings to MCAP, then loader verification | `yam/episode.py` and its read-back tests; swap in ABC's `export_mcap.py` encoding once verified | the sides are physical and never derivable; a wrong default mirrors the whole dataset silently ([FINDINGS §70.13](FINDINGS.md)) |
