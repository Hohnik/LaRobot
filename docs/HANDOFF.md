...
| 6b | ⭐ **Smooth the path at speed — the rate limiter is the bottleneck, measured** | Julien, 2026-08-11: *"at high speeds the arm takes longer to follow the path… I can only really control it at speeds of less than half a meter per second."* **Diagnosed:** `SafeRobot.max_speed` caps every commanded joint at **1.0 rad/s**, but the IK asks for **3.3 rad/s at only 0.25 m/s**, 5.3 at 0.4, 7.8 at 0.6. So above ~0.15 m/s the limiter throttles by 3× or more and the command lags reality by design. ⛔ **Do not simply raise the cap** — it is the guard that turns "dangerous" into "catchable" (FINDINGS, `SafeRobot`). The real fix is to stop asking for step changes: **slew-rate-limit the twist itself** (ramp the puck input rather than the joint output) so the commanded path is achievable, and only then reconsider the cap. `MAX_JOINT_STEP` (1.5 rad/s) is the second, looser limit |
| 6c | ⭐ **Mirror mode — hand-guide one arm, the other copies it** | Julien's idea, 2026-08-11: *"be able to move one of the arms in the guide mode and have the second arm just mirror the exact movements with zero latency."* ⭐ **This is much easier than bimanual teleop and is the right first two-arm feature**: read arm B's measured joint positions and command them straight to arm G. **No IK, no second puck, no cartesian anything** — joint-space copy at 100 Hz. It exercises the two-arm process end to end while the risky parts are absent, which makes it the ideal shakedown for `ArmSession` before real bimanual teleop. Open question: mirrored (some joints negated, for arms facing each other) or identical? That depends on how the two are physically placed |
