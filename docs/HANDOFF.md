...
|---|---|---|
| 0 | ⚠️ **Confirm on the arm: keys, and the park verdict** | ✅ Ctrl-C→park→disable is **confirmed working** (2026-08-12). Still unconfirmed: **keys are no longer swallowed** — press several quickly (`h` then `t` then `h`) and every one should act, which was broken until [FINDINGS §26](FINDINGS.md); and a park that stops ~0.02-0.05 rad short should now say **PARKED … as close as the arm holds itself** rather than STALLED. The **55 °C warning** should appear on any run that works the shoulder. The **blind-thermal stop** cannot be triggered without unplugging CAN mid-session, so it stays unverified — stated rather than assumed |
