...
|---|---|---|
| 0 | ⚠️ **Confirm the three guard fixes on the arm** ([FINDINGS §24](FINDINGS.md)) | Cheap, and it gates item 2. **Ctrl-C** during a live session must now go to the consent flow, not release the arm — press it once, expect `p / g / d`, press it twice to force out. The **55 °C warning** should appear during any run that works the shoulder. The **blind-thermal stop** cannot be triggered on purpose without unplugging CAN mid-session, so it stays unverified; that is stated rather than assumed |
