# Historical status renderer source

Source from `216fda0` (trailing whitespace removed), before detached display snapshots. This is history,
not a current contract: the old claim that display failures were locally caught
was false. Current read failures propagate to application fault cleanup.
