...
"""Diff every readable register on both arms against each other.
    ...
    uv run checks/check_arms_match.py            # dry run — prints the plan, sends nothing
    uv run checks/check_arms_match.py --yes      # transmits register reads only
...
def save_baseline(arms: dict[str, dict[int, dict]], registers: list[str], skipped: dict) -> None:
    ...
    payload = {
        ...
        "registers": registers,
        "skipped": skipped,
        "note": "Baseline for checks/check_arms_match.py. Every value is a register READ "
