...
"""What state is every device on this rig in? One command, one pasteable answer.
    ...
    uv run checks/check_rig.py --raw      # also list hubs and everything else
...
motor is alive — that needs a poll, which transmits. `apps/probe_can.py` is the next step
up and is still silent; `apps/ping_motors.py` transmits and is Julien's to run.
"""
...
def main() -> int:
    ...
    print("  ⚠️ That does NOT prove a motor is alive. This never transmits, so it cannot. "
          "The next\n     silent step is `uv run apps/probe_can.py`; a poll transmits "
