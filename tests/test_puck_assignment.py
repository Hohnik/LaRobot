...
def test_a_single_shared_puck_follows_the_selection() -> None:
    """One reader per cycle follows B, G, then BOTH through real operator keys."""
    from test_session_recording_failures import exercise_shared_puck_selection
    exercise_shared_puck_selection()
    src = (REPO / "apps" / "teleop_session.py").read_text()
