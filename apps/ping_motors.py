...
"""Ask each YAM motor whether it is there. FIRST SCRIPT THAT EVER TRANSMITS.
    ...
    uv run apps/ping_motors.py            # dry run — prints the plan, sends nothing
    uv run apps/ping_motors.py --yes      # actually transmits
...
def gripper_frame_report(arm: str, raw_pos: float) -> list[str]:
    ...
    if limits is None:
        return [f"    ⚠️  no saved jaw limits for arm {arm} — run apps/calibrate_gripper.py"]
    ...
    if shifted is None:
        return [
            ...
            "          not in and held against a stop. Re-run apps/calibrate_gripper.py.",
        ]
...
def main() -> int:
    ...
    if faulted:
        ...
        print(
            ...
            "   To clear them deliberately:  uv run apps/ping_motors.py --arm "
