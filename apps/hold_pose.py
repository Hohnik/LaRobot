...
"""Gravity compensation, the careful way: tell the arm to STAY EXACTLY WHERE IT IS.
    ...
    uv run apps/hold_pose.py                       # dry run: explains the plan
    uv run apps/hold_pose.py --yes                 # LIVE — the arm holds itself
    uv run apps/hold_pose.py --yes --zero-gravity  # LIVE — back-drivable, push it by hand
...
def main() -> int:
    ...
    print(f"  jaw limits : {saved if saved else 'NONE SAVED — run apps/calibrate_gripper.py --yes first'}")
