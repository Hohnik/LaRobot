...
"""Run the teleop loop against the SIMULATED arm. Touches no hardware, ever.
    ...
    uv run apps/teleop_sim.py --demo                 # scripted circle, no input device
    uv run apps/teleop_sim.py                        # driven by the real SpaceMouse
    uv run apps/teleop_sim.py --view                 # ...with the MuJoCo viewer
...
def twist_from_axes(axes: list[float], axis_map: AxisMap) -> np.ndarray:
    ...
    `apps/teleop_gripper.py`. Only the scaling is a teleop concern.
...
def main() -> int:
    ...
    if args.view:
        ...
        try:
            ...
            print("     uv run mjpython apps/teleop_sim.py --view")
