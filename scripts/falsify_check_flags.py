#!/usr/bin/env python3
"""Can `scripts/check_flags.py` actually SEE a broken documented command?

    uv run scripts/falsify_check_flags.py
...
CASES = [
    ...
    ("a word where a float is required",
     "`uv run scripts/teleop_session.py --yes --arm B --max-speed fast`", True),
    ("a mirror mode outside choices",
     "`uv run scripts/teleop_session.py --yes --arms B,G --mirror sideways`", True),
    ("a bad arm inside a comma list, via the plural rule",
     "`uv run scripts/teleop_session.py --yes --arms B,Q`", True),
    ...
    ("a chain, whose second command must not be blamed on the first",
     "`uv run scripts/check_rig.py && uv run scripts/ping_motors.py --arm B --yes`",
     False),
    ("a legitimate placeholder",
     "`uv run scripts/ping_motors.py --arm <B|G> --yes`", False),
]
...
def main() -> int:
    print("Falsifying scripts/check_flags.py — every break must be reported, and every\n"
