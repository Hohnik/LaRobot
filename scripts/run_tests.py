...
⛔ WHY THIS EXISTS ([FINDINGS §67.5](../docs/FINDINGS.md)): each `scripts/test_*.py` carries its own `main()`, nothing ran them as a whole, and two files sat red for days — one a real regression the tests had caught and nobody collected. A suite that is never run as a whole is a fixture with no counter.
...
def main() -> int:
    ...
    ap.add_argument("--dir", default=str(REPO / "scripts"),
